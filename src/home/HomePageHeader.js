...
export default class HomePageHeader extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(`https://classcast-198812.appspot.com/users/whatsnew_all/`+`whatsnew_the_optimist/`)
      .then((response)=>{
  ...
  onAction =(item) => {
    console.log("snsandasna: "+JSON.stringify(item));
    if(item.type == 'video'){
  ...
  render () {
    console.log("dasnjlasbkjabs: "+this.state.ActiveSlide)
    return(
      <View style={styles.mainContainer}>
        ...
        />
        <Pagination
          dotsLength={this.state.updates.length}
          activeDotIndex={this.state.ActiveSlide}
          containerStyle={{paddingBottom: 0.01* screen.height, paddingTop: 0.01* screen.height, alignSelf: 'center'}}
          dotColor={'rgba(0, 0, 255, 0.92)'}
