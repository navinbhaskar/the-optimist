...
class newLoadingTest extends Component {
  ...
  async componentDidMount() {
    console.log("dbidbwkdqwd: "+JSON.stringify(this.props.navigation.state.params.test_id));
    BackHandler.addEventListener('hardwareBackPress', this.handleBackButton);
    
    if(this.props.navigation.state.params.path == 'TabB_test') {
      var currentUser = await firebase.auth().currentUser;               
      console.log("emoiid: "+currentUser['phoneNumber'].slice(3, 13));
      axios.get(`https://classcast-198812.appspot.com/teachersapp/fetchTestData/`+this.props.navigation.state.params.test_id)
        .then(res=> {
          console.log("bwabskjsAA: "+JSON.stringify(res.data));
          console.log("bwabskjsAA: "+JSON.stringify(res.data.performance.map(user=> user.username)  ));
          arr = res.data.performance.filter(user=> {return (user.username == currentUser['phoneNumber'].slice(3, 13))} );
          console.log("bwabskjsBB: "+JSON.stringify(arr.length));
          if(arr.length > 0){
          ...
          this.setState({duration: res.data.blocks.length * 20})
        })
    }

    if(this.props.navigation.state.params.path == 'TabB') {
      data = {
        "subject": this.props.navigation.state.params.subject,
        "chapter": this.props.navigation.state.params.chapter_name
      }
      console.log("bikkbkbkbkbkb: "+JSON.stringify(data));
    ...
    else if(this.props.navigation.state.params.chapterAvailable) {
      ...
      return(
        <View style={styles.container}>
          ...
          <View style={[styles.tncHeader, {justifyContent: 'space-around'}]}>
            <Text style={{fontFamily: 'Montserrat-SemiBold', fontSize: 4 * vw, color: 'black'}}>{this.state.duration + ' mins'}</Text>
            <Text> - </Text>
            <Text style={{fontFamily: 'Montserrat-SemiBold', fontSize: 4 * vw, color: 'black'}}>Maximum Marks: {this.state.maximum_marks}</Text>
          </View>
