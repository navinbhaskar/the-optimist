...
class buynow extends Component {
  ...
  async componentDidMount() {
    console.log("sbsnakasjsd: "+JSON.stringify(this.props.navigation.state.params));
    BackHandler.addEventListener('hardwareBackPress', this.handleBackButton);

	axios.get('https://classcast-198812.appspot.com/white_label/get_packages/'+this.props.navigation.state.params.data.course_id)
            .then(res => {
  ...
  _renderItem = ({item, index}) => (
      <TouchableNativeFeedback
			onPress={() => {
				...
				console.log("dasnsanak: "+JSON.stringify(item))
			}}
		>
			<View style={{height: 50 * vw, width: 25 * vw, borderWidth: 0.5 * vw, borderRadius: 2 * vw, borderColor: '#035493', backgroundColor: index == this.state.selectedPackageIndex ? '#035493': 'white', marginBottom: 5 * vh, marginRight: 10 * vw}}>
  ...
  render() {
    console.log("sanlksandn: "+JSON.stringify(this.state.package))
    ...
    return (
        <View style={{width: '100%', height: '100%'}}>
        	<View style={{ height: '92%', width: '100%'}}>
	        	<ScrollView>
	            	<View style={{height: 30 * vh, width: 100 * vw, alignItems:'center', justifyContent:'center', alignSelf:'center'}}>
		            	<Image
		       		...
		       		<View style={{width: '90%', marginTop: 5 * vh, alignSelf: 'center'}}>
	                	<Text style={{ fontFamily:'Montserrat-SemiBold', color: '#333333', fontSize: 4*vw, textAlign: 'center'}}>{this.props.navigation.state.params.data.course_details}</Text>
	            </View>
                ...
                <TouchableNativeFeedback
		            ...
		            >
		            	<Text style={{ fontFamily:'Montserrat-SemiBold', color: '#035493', fontSize: 5*vw, textAlign: 'center', marginTop: 5 * vh, marginBottom: 5 * vh, textDecorationLine: 'underline'}}>Have an Access code?</Text>
		            </TouchableNativeFeedback>
		            <View style={{flexDirection: 'row', width: '90%', alignSelf: 'center', marginTop: 5 * vh, alignSelf: 'center'}}>
		            	{ this.state.isReady &&
				            <FlatList
				              horizontal={true}
        	...
        	<TouchableNativeFeedback
        		onPress={() => {
				  var options = {
				    description: "S P Sharma Classes",
				    image: 'https://storage.googleapis.com/sp_sharma_classes/ic_launcher.png',
				    currency: 'INR',
				    key: 'rzp_live_TNKvMB4toFyCYe',
				    amount: this.state.package[this.state.selectedPackageIndex].amount * 100,
				    name: this.props.navigation.state.params.data.course_name,
				    prefill: {
            ...
            // handle success
            console.log("snkjasndska: "+JSON.stringify(data));
            ...
            json_data = {
              ...
              "payement_mode": "Razorpay",
              "payment_id": data['razorpay_payment_id'],
              "package_index": this.state.selectedPackageIndex,
              "package_id": this.state.package[this.state.selectedPackageIndex].package_id
            }
				    axios.post(`https://classcast-198812.appspot.com/white_label/add_course_enrollment_with_package/`, json_data)
		                .then(function (response){
		                	console.log("paymentSuccessEnrollmentsuccess");
		                  console.log("sdjsadjksaesfsfddsds: "+JSON.stringify(response.data));
		                }.bind(this))
		                .catch(function (error) {
                      console.log("sdjsadjksaesfsfddsdsserror: "+JSON.stringify(error));
		                  console.log('error');
		                });
        	...
        	>
			    <View style={{ height: '8%', width: '100%', backgroundColor: '#035493', alignItems: 'center', justifyContent: 'center'}}>
			    	{ this.state.isReady &&
			    	<Text style={{ fontFamily:'Montserrat-Bold', color: 'white', fontSize: 5*vw, textAlign: 'center'}}> Join for ₹{this.state.package[this.state.selectedPackageIndex].amount} </Text>
			    	}
			    </View>
			</TouchableNativeFeedback>
        <Modal
                ...
                <TouchableNativeFeedback
                    ...
                    axios.post('https://classcast-198812.appspot.com/white_label/enrollment_from_token_new/', data)
                      .then((response) => 
                      {
                        ...
                        console.log("sdjsadjksaesfsfddsds1:"+JSON.stringify(response.data));
                      ...
                      .catch(err=>{
                        this.setState({ accessCodeProcess: false });
                        console.log("bkjbkjb: "+JSON.stringify(err));
                      })
...

      {
        <Modal
          ...
          <View style={[styles.tncModal2,{ alignItems: 'center'}]}>
            ...
            <Text style={[styles.tncHeadingBig,{fontFamily: 'Montserrat-Regular', textAlign: 'center', fontSize: 4 * vw}]}>Your Payment was Successfull. You can check your course in My Courses Section on Home Page</Text>
            <TouchableNativeFeedback
              onPress={()=>{
                this.setState({paymentSuccessModal: false});
                this.props.navigation.navigate('Home')
