...
class buynowtest extends Component {
	                ...
	                <View style={{flexDirection: 'row', justifyContent: 'space-between', width: '90%', alignSelf: 'center', marginTop: 5 * vh}}>
                  		{
                    		this.state.package && this.state.package.map((blocks, index)=>{
                                ...
                                this.setState({selectedPackageIndex: index})
                    				...
                    				>
	                    				<View style={{height: 50 * vw, width: 25 * vw, borderWidth: 0.5 * vw, borderRadius: 2 * vw, borderColor: '#035493', backgroundColor: index == this.state.selectedPackageIndex ? '#035493': 'white', marginBottom: 5 * vh}}>
	                    					...
	                    					<View style={{marginTop: 4 * vw}}>
	                    						<Text style={{fontSize: 4 * vw, color: index == this.state.selectedPackageIndex? 'white': '#035493', fontFamily: 'Montserrat-SemiBold', textAlign: 'center'}}>{'₹ '+blocks.amount}</Text>
	                    					</View>
	                    					<View style={{marginTop: 4 * vw}}>
	                    						<Text style={{fontSize: 3 * vw, color: index == this.state.selectedPackageIndex? 'white': '#035493', fontFamily: 'Montserrat-SemiBold', textAlign: 'center'}}>{blocks.name}</Text>
	                    					</View>
	                    				</View>
	                    			</TouchableNativeFeedback>
                    			)
...

      {
        <Modal
          ...
          <View style={[styles.tncModal2,{ alignItems: 'center'}]}>
            ...
            <Text style={[styles.tncHeadingBig,{fontFamily: 'Montserrat-Regular', textAlign: 'center', fontSize: 4 * vw}]}>Your Payment was Successfull.</Text>
            <TouchableNativeFeedback
              onPress={()=>{
                this.setState({paymentSuccessModal: false});
                this.props.navigation.navigate('Home')
