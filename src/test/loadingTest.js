...
class testInstructions extends Component {
  ...
  async componentDidMount() {
    console.log("idndslndslas: "+JSON.stringify(this.props.navigation.state.params));
    BackHandler.addEventListener('hardwareBackPress', this.handleBackButton);
    
      axios.get(`https://classcast-198812.appspot.com/white_label/get_test_data/`+this.props.navigation.state.params.data.test_id)
        .then(res=> {
          console.log("bwabskjsAA: "+JSON.stringify(res.data));
          console.log("bwabskjsAA: "+JSON.stringify(res.data.end_date));
          this.setState({
            data: res.data,
  ...
  render () {
    //console.log("bwabskjsAA: "+moment(this.state.data.end_date, "YYYY-MM-DDTHH:MM:SS.ffff"));
    //console.log("bwabskjsAA: "+new Date(this.state.data.end_date, "YYYY-MM-DDTHH:MM:SS.ffff"));
    console.log("samlaskdmsadm: "+JSON.stringify(this.props.navigation.state.params))
      ...
      return(
        <View style={styles.container}>
          ...
          <Text style={{fontFamily: 'Montserrat-SemiBold', fontSize: 5 * vw, color: 'black'}}></Text>
            
          </View>
          <View style={[styles.tncHeader, {justifyContent: 'space-around'}]}>

            <Text style={{fontFamily: 'Montserrat-SemiBold', fontSize: 4 * vw, color: 'black'}}>{Math.floor(this.props.navigation.state.params.data.duration/60)}:{(this.props.navigation.state.params.data.duration % 60) > 9 ? this.props.navigation.state.params.data.duration % 60 : '0'+ this.props.navigation.state.params.data.duration % 60+ ' mins'}</Text>
            <Text> - </Text>
            <Text style={{fontFamily: 'Montserrat-SemiBold', fontSize: 4 * vw, color: 'black'}}>Maximum Marks: {this.props.navigation.state.params.data.no_of_questions * this.props.navigation.state.params.data.pos_marks} </Text>
          </View>
          <ScrollView style={{width: '100%', padding: 19 }}>
            ...
            <Text style={{fontFamily: 'Montserrat-Regular', fontSize: 4 * vw, color: 'black'}}>3: {this.props.navigation.state.params.negative_marking ? this.props.navigation.state.params.neg_marks: 0} mark(s) will be deducted for every wrong answer.</Text>
            ...
            <TouchableNativeFeedback
               onPress={() => {
                const navigateAction = NavigationActions.navigate({
                  routeName: this.props.navigation.state.params.path == 'paid' ? 'ongoingTest': 'ongoingTest1',
                  params: {
                    data: this.state.data,
                    test_data: this.props.navigation.state.params.data,
