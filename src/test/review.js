...
class review extends Component {
  ...
  render () {
      console.log("sasdldsla : "+JSON.stringify(this.props.navigation.state.params ));
      return(
        <View style={styles.container}>
          <View style={styles.testHeader}>
            <View style={{ justifyContent: 'center', alignItems: 'center', marginLeft: 2 * vw}}>
              <Progress.Pie progress={(this.props.navigation.state.params.timer)/(this.props.navigation.state.params.test_data.duration)} size={6 * vw} color={'white'} />
            </View>
            <View style={{flexDirection: 'column', alignSelf: 'center'}}>
              <Text style={styles.headerText}>Timer: {Math.floor((this.props.navigation.state.params.timer)/60)}:{((this.props.navigation.state.params.timer) % 60) > 9 ? (this.props.navigation.state.params.timer) % 60 : '0'+ (this.props.navigation.state.params.timer) % 60}</Text>
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            <TouchableNativeFeedback onPress={()=> this.setState({showSubmitModal: true})}>              
              <View style={{height: 5 * vh, width: '19%', backgroundColor: '#323131', alignSelf: 'center', justifyContent: 'center', borderRadius: 4 * vw, alignItems: 'center'}}>
                <Text style={{fontFamily: 'Montserrat-Regular', fontSize: 3 * vw, color: 'white'}}>Exit</Text>
              </View>
            </TouchableNativeFeedback>
          </View>
          { this.state.loading &&
            <Modal 
              ...
              <View style={{height: 18 * vh, width: '70%', position: 'absolute', alignSelf: 'center', borderRadius: 2*vw, backgroundColor: 'white'}}>
                <View style={{height: 10 * vh, width: '100%', justifyContent: 'center'}}>
                  <Text style={{fontFamily: 'Montserrat-SemiBold', fontSize: 4 * vw, marginLeft: 2 * vw, marginRight: 2 * vw, color: 'black'}}> Are you sure you want to exit the review? </Text>
                </View>
                ...
                return(
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    ...
                    <View style={[styles.textContainer, {marginTop: 1 * vh}]}>
                      { this.state.loading &&
                      <RenderQuestions
                        data={{text: "<p>"+options.value+"</p>", color: (this.state.blocks['sections'][this.state.selectedSection].blocks[this.state.questionIndex].correctOption.filter(function (optionvalue) { return (optionvalue == index+1) }).length > 0) ? '#80e180': (this.state.blocks['sections'][this.state.selectedSection].blocks[this.state.questionIndex].correctOption.filter(function (optionvalue) { return (optionvalue == index+1) }).length > 0) ? '#f28888' : 'white' }}
                        />
                      }
                    </View>
                  </View>
                ...
                return(
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    ...
                    <View style={[styles.textContainer, {marginTop: 1 * vh}]}>
                      { this.state.loading &&
                      <RenderQuestions
                        data={{text: "<p>"+options.value+"</p>", color: this.props.navigation.state.params.data.sections[this.state.selectedSection].blocks[this.state.questionIndex].correctOption == (index).toString() ? '#80e180': (this.props.navigation.state.params.data.sections[this.state.selectedSection].blocks[this.state.questionIndex].selectedOption == (index+1).toString() ? '#f28888': 'white') }}
                        />
                      }
                    </View>
                  </View>
